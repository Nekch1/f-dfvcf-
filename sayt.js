...
function addToFavorites(button) {
    ...
    const title = catalogCard.getAttribute('data-title');
    const recipeLink = catalogCard.getAttribute('data-link');
    const info = catalogCard.querySelector('.catalog-card-info');
    const details = Array.from(info.children).map(div => div.textContent.trim());
...
function displayFavorites() {
    ...
    // Отображаем все рецепты из избранного
