...
function previewImage(event) {
    ...
    if (file) {
        ...
        reader.onload = function() {
            imagePreview.src = reader.result; // Устанавливаем изображение в контейнер
            previewContainer.style.display = 'inline-block'; // Показываем контейнер с изображением
        }

        reader.readAsDataURL(file); // Читаем файл как DataURL
    } else {
        previewContainer.style.display = 'none'; // Скрываем контейнер, если файл не выбран
    }
}

// Обработчик клика по элементу списка рецептов
document.getElementById('recipeListtt').addEventListener('click', function(event) {
    const clickedItem = event.target.closest('li'); // Получаем кликнутый элемент списка

    if (clickedItem) {
        // Если элемент не был выделен, выделяем его
